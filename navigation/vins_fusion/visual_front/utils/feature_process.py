...
class PointTracker(object):
    ...
    def nn_match_two_way(self, desc1, desc2, nn_thresh=0.7):
        ...
        keep_bi = torch.arange(len(idx), device=device) == idx2[idx]
        keep = torch.logical_and(keep, keep_bi)
        idx = idx[keep]
        scores = scores[keep]
        # Get the surviving point indices.
        m_idx1 = torch.arange(desc1.shape[1], device=device)[keep]
        m_idx2 = idx
        # Populate the final 3xN match data structure.
        matches = torch.zeros((3, keep.sum()), device=device)
        matches[0, :] = m_idx1
        matches[1, :] = m_idx2
        matches[2, :] = scores
