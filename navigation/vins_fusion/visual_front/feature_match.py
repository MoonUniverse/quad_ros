...
class VisualTracker:
    ...
    def readImage(self, new_img):
        ...
        assert (new_img.ndim == 2 and new_img.shape[0] == self.height and new_img.shape[1] ==
                self.width), "Frame: provided image has not the same size as the camera model or image is not grayscale"
        ...
        self.forwframe_['keyPoint'], self.forwframe_[
            'descriptor'], heatmap = self.SuperPoint_Ghostnet.run(self.new_frame, conf_thresh=0.015)
        ...
        if not self.no_display:
            out1 = (np.dstack((self.curframe_['image'], self.curframe_[
                    'image'], self.curframe_['image'])) * 255.).astype('uint8')
            for i in range(len(self.curframe_['PointID'])):
                pts1 = (int(round(self.curframe_[
                        'keyPoint'][0, i]))-3, int(round(self.curframe_['keyPoint'][1, i]))-3)
                pts2 = (int(round(self.curframe_[
                        'keyPoint'][0, i]))+3, int(round(self.curframe_['keyPoint'][1, i]))+3)
                pt2 = (int(round(self.curframe_['keyPoint'][0, i])), int(
                    round(self.curframe_['keyPoint'][1, i])))
            ...
            cv2.putText(out1, 'pre_image Point', (4, 12),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), lineType=16)

            out2 = (np.dstack((self.forwframe_['image'], self.forwframe_[
                    'image'], self.forwframe_['image'])) * 255.).astype('uint8')
            for i in range(len(self.forwframe_['PointID'])):
                pts1 = (int(round(self.forwframe_[
                        'keyPoint'][0, i]))-3, int(round(self.forwframe_['keyPoint'][1, i]))-3)
                pts2 = (int(round(self.forwframe_[
                        'keyPoint'][0, i]))+3, int(round(self.forwframe_['keyPoint'][1, i]))+3)
                pt2 = (int(round(self.forwframe_['keyPoint'][0, i])), int(
                    round(self.forwframe_['keyPoint'][1, i])))
            ...
            cv2.putText(out2, 'cur_image Point', (4, 12),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), lineType=16)
